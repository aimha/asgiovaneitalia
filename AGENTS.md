...
and SASS for styles. Deployed in two environments
...
│   ├── MouseHighlight/         # Mouse-driven highlight effect
│   ├── Slider/                 # Image/content slider
│   └── PageComponents/
│       ├── About/              # About section
│       ├── Activities/         # Activities cards (Sport, Cultura, Musica, Ristorazione)
│       ├── Footer/             # Site footer
│       ├── Hero/               # Hero banner
│       ├── History/            # History timeline
│       ├── Membership/         # Membership/CTA section
│       └── Where/              # Location/address section
├── data/
│   └── stores/
│       └── Store.jsx           # Shared reactive state (single store)
├── js/                         # Utility functions and classes
│   ├── animation.js            # AnimationClass for timeline-based animations
│   ├── cubicBezier.js          # Easing curves
│   └── intersectionObserver.js # Scroll-based visibility observer
├── routes/
...
- `ComponentName.jsx` — SolidJS component
- `ComponentName.module.js` — JS logic class (DOM manipulation, animations)
- `ComponentName.module.scss` — Scoped SASS styles
...
```bash
npm run dev        # or npm run start (alias)
```
...
- **`.module.js` files** contain component logic classes exported as default — they handle DOM
  manipulation, animations, and imperative logic. They receive the component root element and
  CSS module styles as constructor arguments
...
1. **Plan first.** Describe what you intend to do, which files you will touch, and why. Wait for
...
3. **Run lint after changes.** Execute `npm run lint` (or `npm run lint:fix` to auto-fix) to
   ensure code quality before considering the task done.
4. **Write a summary when done.** At the end of each task, produce a brief `docs/summaries/<task-name>.md`
...
5. **Do not fix unrelated issues.** If you notice something broken outside the scope of the task,
